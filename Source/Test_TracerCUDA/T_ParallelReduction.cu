...
#include "TracerCUDA/GPUPrimitiveTriangle.h"
...
TEST(ParallelReduction, Generic)
{
    ...
    // Integer Kernel
    int resultInt = 0;
    ReduceArrayGPU<int, ReduceAdd<int>, cudaMemcpyDeviceToHost>
    (
        resultInt,
...
TEST(ParallelReduction, CustomFunction)
{
    ...
    // Float Kernel
    float resultFloat = 0;
    ReduceArrayGPU<float, CustomReduce, cudaMemcpyDeviceToHost>
    (
        resultFloat,
        static_cast<float*>(fDataIn),
        ElementCount,
...
TEST(ParallelReduction, LargeStruct)
{
    ...
    ReduceArrayGPU<Matrix4x4f, ReduceAdd, cudaMemcpyDeviceToHost>
    (
        resultMatrix,
