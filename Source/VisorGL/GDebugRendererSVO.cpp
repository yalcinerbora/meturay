                                ...
                                uint32_t nodeIndex, bool isLeaf) const
{
    ...
    auto FindNthSet = [=](uint32_t input, uint32_t n) -> uint32_t
    {
        ...
        uint32_t bitCount = static_cast<uint32_t>(Utility::BitCount(input));
        if(bitCount < n)
        {
            assert(bitCount >= n);
                                          ...
                                          uint32_t depth)
{
    ...
    const gl::GLuint workCount = curTexture.Size()[1] * curTexture.Size()[0];
    ...
    compReduction.Bind();
    // Bind Uniforms
    gl::glUniform2ui(U_RES, curTexture.Size()[0], curTexture.Size()[1]);
    // Bind SSBO
    ...
    // Images
    gl::glBindImageTexture(I_OUT_REF_IMAGE, currentTexture.TexId(),
    ...
    gl::GLuint gridX_2D = (curTexture.Size()[0] + WORK_GROUP_2D_X - 1) / WORK_GROUP_2D_X;
    gl::GLuint gridY_2D = (curTexture.Size()[1] + WORK_GROUP_2D_Y - 1) / WORK_GROUP_2D_Y;
