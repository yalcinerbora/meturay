                                ...
                                int ElementCount)
{
   ...
   Vector2 v3 = Vector2(fInput + gId * 4);
   Vector3 v4 = Vector3(fInput + gId * 4);
   Vector4 v5 = Vector4(fInput + gId * 4);
...
TEST(VectorGPU, Construction)
{
    ...
    Vector4* h_data;
    CUDA_CHECK(cudaMalloc(&h_data, vectorSize));

    // Kernel Call
    KConstruction<<<1,1>>>(h_data);
    CUDA_KERNEL_CHECK();
...
TEST(VectorGPU, Operators)
{
    ...
    Vector4* h_data;
    CUDA_CHECK(cudaMallocManaged(&h_data, vectorSize));

    // Kernel Call
    KOperators<<<1,1>>>(h_data);
    CUDA_KERNEL_CHECK();
    CUDA_CHECK(cudaDeviceSynchronize());
