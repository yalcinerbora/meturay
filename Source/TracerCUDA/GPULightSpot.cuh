...
inline bool CPULightGroupSpot::RequiresLuminance() const
{
    return false;
}

const std::vector<HitKey>& CPULightGroupSpot::AcquireMaterialKeys() const
{
    return hHitKeys;
}

inline TracerError CPULightGroupSpot::GenerateLumDistribution(const std::vector<std::vector<float>>& luminance,
                                                              const std::vector<Vector2ui>& dimension,
                                                              const CudaSystem&)
{
