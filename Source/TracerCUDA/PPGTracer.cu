...
void PPGTracer::Finalize()
{
    ...
    if(currentTreeIteration == nextTreeSwap)
    {
        ...
        uint64_t currentSTreeSplitThreshold64 = static_cast<uint64_t>((std::pow(2.0f, treeSwapIterationCount) *
                                                                     options.sTreeSplitThreshold));
        currentSTreeSplitThreshold64 = std::min<uint64_t>(currentSTreeSplitThreshold64, std::numeric_limits<uint32_t>::max());
        uint32_t currentSTreeSplitThreshold = static_cast<uint32_t>(currentSTreeSplitThreshold64);
